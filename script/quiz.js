...
for(let i = 0; i < 6; i++) {
    document.querySelectorAll('.textBir')[i].innerHTML = birdsData[globalCount][i].name
    if( i === 6){
        i = 0
    }
}
...
document.addEventListener('click', function(el){
    if(el.target.id === '0' || el.target.id === '1' || el.target.id === '2' || el.target.id === '3' || el.target.id === '4' || el.target.id === '5'){
        document.querySelector('.startText').style.display = 'none'
        document.getElementById('birdImgBottom').src = birdsData[globalCount][el.target.id].image
        document.getElementById('birdName').textContent = birdsData[globalCount][el.target.id].name
        document.getElementById('birdDwells').textContent = birdsData[globalCount][el.target.id].species
        document.getElementById('textBird').textContent = birdsData[globalCount][el.target.id].description
        document.getElementById('birdVoice').src = birdsData[globalCount][el.target.id].audio
...
function quiz(random) {
  console.log(random);
  document.getElementById('quizVoice').src = birdsData[globalCount][random].audio
  document.addEventListener('click', function countFunction(el){
    if(el.target.id === String(random)){
      document.getElementById('quizBirdName').textContent = birdsData[globalCount][random].name
      document.getElementById('birdQuizImg').src = birdsData[globalCount][random].image
      document.querySelectorAll('.span')[random].style.background = 'green'
      new Audio('/SongBird/assets/images/assets_images_prav.mp3').play()
      document.querySelector('.button').style.background = 'green'
      count += 5
      localStorage.setItem('digital', count)
      document.querySelector('.score').textContent = `Score: ${count}`
    }else if(el.target.id != String(random) && (el.target.id === '0' || el.target.id === '1' || el.target.id === '2' || el.target.id === '3' || el.target.id === '4' || el.target.id === '5')) {
      document.querySelectorAll('.span')[el.target.id].style.background = 'red'
      new Audio('/SongBird/assets/images/assets_images_neprav.mp3').play()
      count -= 1
    }
    if(document.querySelectorAll('.span')[random].style.background === 'green'){
      document.removeEventListener('click', countFunction)
    }
  })
}

quiz(getRandomInt(0, 6))

document.querySelector('.button').addEventListener('click', function(){
  if (document.querySelector('.button').style.background === 'green' && globalCount < 5) { 
    globalCount += 1
    document.querySelectorAll('.classList')[globalCount].classList.add('active') 
    document.querySelectorAll('.classList')[globalCount - 1].classList.remove('active') 
    document.querySelector('.button').style.background = '#303030'
    document.querySelector('.selectedOption').style.display = 'none'
    document.querySelector('.startText').style.display = 'block'
    document.querySelector('.textBird').textContent = ''
    document.getElementById('quizBirdName').textContent = '*****'
    document.getElementById('birdQuizImg').src = '../assets/images/bird.jpg'
    for(let i = 0; i < 6; i++) {
      document.querySelectorAll('.span')[i].style.background = '#444'
    }
    for(let i = 0; i < 6; i++) {
      document.querySelectorAll('.textBir')[i].innerHTML = birdsData[globalCount][i].name
    }
    quiz(getRandomInt(0, 6))
      ...
      window.open('/SongBird/pages/end.html')
    }
})
